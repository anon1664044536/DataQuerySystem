...
public class NL2PY2SQLEvaluator {
    ...
    public static void main(String[] args) {
        ...
        String predictionsFile = "spider/predictions_nl2py2sql.txt";
        String metricsFile = "spider/metrics_nl2py2sql_log.txt";

        if (DataManager.apiKey().isBlank() || DataManager.mqlPYSQLAppId().isBlank() || DataManager.sqlPYSQLAppId().isBlank()) {
            System.err.println("Error: DashScope API key or NL2PY/PY2SQL App ID is missing.");
            return;
        }

        // 复用 AskingAgent 的 API 调用能力。这里将 mqlAppId 当作 NL2PY 的 ID，sqlAppId 当作 PY2SQL 的 ID
        AskingAgent agent = new AskingAgent(DataManager.apiKey(), DataManager.mqlPYSQLAppId(), DataManager.sqlPYSQLAppId());
        ...
        try {
            ...
            metricsList.add(String.format("%-5s | %-15s | %-15s | %-20s | %-15s | %-20s | %-15s",
                    "Q_ID", "DB_ID", "NL2PY_Time(ms)", "NL2PY_Tokens", "PY2SQL_Time(ms)", "PY2SQL_Tokens", "Total_Time(ms)"));
            ...
            try (PrintWriter predWriter = new PrintWriter(new OutputStreamWriter(new FileOutputStream(predictionsFile), "UTF-8"));
                ...
                for (TestCase tc : testCases) {
                    ...
                    // Stage 1: NL -> Python
                    // =====================================
                    String nl2pyInput = "【当前数据库 Schema】\n" + schemaText + 
                                        "\n\n【用户问题】\n" + tc.question;

                    long nl2pyStart = System.currentTimeMillis();
                    ApplicationResult pyResult = null;
                    String pyText = "";
                    int pyTokens = 0;
                    try {
                        pyResult = agent.getMqlResult(nl2pyInput);
                        pyText = pyResult.getOutput().getText();
                        pyTokens = extractTokens(mapper, pyResult);
                    } catch (Exception e) {
                        System.err.println("Error in NL2PY: " + e.getMessage());
                    }
                    long nl2pyTime = System.currentTimeMillis() - nl2pyStart;
                    
                    System.out.println("     ✅ NL2PY Done. Python output:\n" + pyText);
                    
                    // =====================================
                    // Stage 2: Python -> SQL
                    // =====================================
                    long py2sqlStart = System.currentTimeMillis();
                    ...
                    if (!pyText.isEmpty()) {
                        try {
                            String py2sqlInput = "【当前数据库 Schema】\n" + schemaText +
                                                 "\n\n【原始问题】\n" + tc.question +
                                                 "\n\n【NL2PY 推理代码】\n" + pyText;

                            sqlResult = agent.getSqlResult(py2sqlInput);
                            sqlText = sqlResult.getOutput().getText();
                            sqlTokens = extractTokens(mapper, sqlResult);
                        } catch (Exception e) {
                            System.err.println("Error in PY2SQL: " + e.getMessage());
                        }
                    }
                    long py2sqlTime = System.currentTimeMillis() - py2sqlStart;

                    long totalTime = nl2pyTime + py2sqlTime;
                    int totalTokens = pyTokens + sqlTokens;
                    ...
                    String metricLine = String.format("%-5d | %-15s | %-15d | %-20d | %-15d | %-20d | %-15d",
                            tc.id, tc.dbId, nl2pyTime, pyTokens, py2sqlTime, sqlTokens, totalTime);
                ...
                System.out.println("\n🎉 Evaluation fully completed. Check " + predictionsFile + " and " + metricsFile);
